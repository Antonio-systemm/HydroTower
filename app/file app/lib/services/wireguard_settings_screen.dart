...
class _WireGuardSettingsScreenState extends State<WireGuardSettingsScreen> {
  ...
  Future<void> _saveConfiguration(String rawConfiguration) async {
    ...
    try {
      ...
      _message('Profilo WireGuard importato.');
    } on FormatException catch (error) {
      if (mounted) setState(() => _error = error.message);
    } catch (error) {
      if (mounted) setState(() => _error = 'Importazione non riuscita: $error');
    } finally {
  ...
  Future<void> _pickFile() async {
    try {
      ...
      await _saveConfiguration(utf8.decode(bytes, allowMalformed: false));
    } catch (error) {
      if (mounted) setState(() => _error = 'Lettura file non riuscita: $error');
    }
  }
  ...
  Future<void> _connect() async {
    ...
    try {
      ...
      if (connected) _message('VPN WireGuard collegata.');
    } catch (error) {
      if (mounted)
  ...
  Future<void> _disconnect() async {
    ...
    try {
      ...
      _message('VPN WireGuard scollegata.');
    } catch (error) {
      if (mounted)
  ...
  Future<void> _deleteConfiguration() async {
    ...
    _message('Configurazione WireGuard eliminata.');
  }
